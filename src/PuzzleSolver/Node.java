...
public class Node {
     ...
     */
    Node(GameState state, Node parent, int cost) {
        this.state = state;
        this.parent = parent;
        this.cost = cost;
    }
     ...
     */
    static Node findNodeWithState(ArrayList<Node> nodeList, GameState gs) {
        for (Node n : nodeList) { // looping through each node in the nodeList
            if (gs.sameBoard(n.state)) return n; // if this node is the same as current argument
        }
        return null;
    }
